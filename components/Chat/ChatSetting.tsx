...
import {
  ...
  Users,
  FileText,
  Image as ImageIcon, // Icon for Media
  Link as LinkIcon, // Icon for Links
  FileBox, // Using FileBox as placeholder for GIF icon
} from 'lucide-react';
...
const dummyMembers = [
  { id: '1', name: 'Nhuận Nguyễn', status: 'online', role: 'admin', avatarUrl: 'https://via.placeholder.com/40' },
  { id: '2', name: 'Wellcare', status: 'last seen recently', role: 'admin', avatarFallback: 'W' },
  { id: '3', name: 'Khoa Nguyễn', status: 'last seen recently', role: 'admin', avatarUrl: 'https://via.placeholder.com/40' },
  { id: '4', name: 'Ân Phạm', status: 'last seen within a month', role: 'member', avatarFallback: 'A' }, // Fixed fallback
];
    ...
    <>
      ...
      <div className="flex flex-col items-center pt-6">
        <Avatar className="w-20 h-20 mb-3">
          ...
          <AvatarFallback className="text-3xl">{groupInfo.avatarFallback}</AvatarFallback>
        </Avatar>
        <h2 className="text-xl font-semibold">{groupInfo.name}</h2>
        <p className="text-sm text-muted-foreground">{groupInfo.memberCount} members</p>
      </div>
               ...
               <li key={member.id} className="flex items-center justify-between px-4 py-3">
                 <div className="flex items-center">
                   <Avatar className="w-10 h-10 mr-3">
                     ...
                     <AvatarFallback>{member.avatarFallback || member.name.charAt(0)}</AvatarFallback>
                   </Avatar>
                   <div>
                     <p className="font-medium">{member.name}</p>
                     <p className={cn(
                       "text-xs",
                       member.status === 'online' ? 'text-green-500' : 'text-muted-foreground'
                     )}>
                       {member.status}
                     </p>
                   </div>
